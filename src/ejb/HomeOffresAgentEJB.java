...
public class HomeOffresAgentEJB {
	 ...
	 * try {
	 * 
	 * InitialContext initialContext = new InitialContext(); javax.sql.DataSource
	 * dataSource = (DataSource) initialContext
	 * .lookup("java:jboss/datasources/MaDSBanqueTest");
	 * 
	 * Connection connection = dataSource.getConnection(); Connection connection2 =
	 * dataSource.getConnection();
	 * 
	 * String email = "john.doe@gmail.com"; String role = "";
	 * 
	 ...
	 * 
	 * while (rsSelectRole.next()) { role = rsSelectRole.getString("role");
	 * System.out.println("Role in while: " + role);
	 * 
	 * if (role.equals("client")) { System.out.println("in if");
	 ...
	 * 
	 * Integer id = 0; Double price = 0.0;
	 * 
	 * while (rsSelect.next()) {
	 * 
	 * id = rsSelect.getInt("id"); System.out.println("id: " + id); price =
	 * rsSelect.getDouble("prix"); System.out.println("prix: " + price); }
	 * 
	 * }
	 * 
	 * else if (role.equals("agent") || role.equals("admin")) {
	 * java.sql.PreparedStatement stGetAgentAdmin =
	 * connection2.prepareStatement("SELECT * FROM offres");
	 * 
	 * ResultSet rsSelect = stGetAgentAdmin.executeQuery();
	 * 
	 * offre = new Offre(); obj = new JSONObject();
	 * 
	 * arrayOffre = new ArrayList<Offre>(); Integer id = 0; // String title = "";
	 * Double price = 0.0;
	 * 
	 * while (rsSelect.next()) {
	 * 
	 ...
	 * 
	 * else { java.sql.PreparedStatement stGetVisiteur = connection2
	 * .prepareStatement("SELECT * FROM offres WHERE idEtat = 1 OR idEtat = 2");
	 * 
	 ...
	 * response.getWriter().append("\nConnexion   la DB ok le ").append(new
	 * Date().toLocaleString()); } catch (Exception e2) {
	 * response.getWriter().append("\nProbl me de connexion   la DB : ").append(new
	 * Date().toLocaleString()); System.out.println(e2.getMessage().toString()); } }
	 */
